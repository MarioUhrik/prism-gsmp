...
\begin{definition}[\cite{Haas:book}] 
	...
	\begin{itemize}
		...
		\item $\suc : \events \rightarrow \transmats(S)$ is a \emph{successor function} that assigns to an event $e$ a~probability matrix %$\suc(e)$
...
%	\setcounter{enumi}{5}
	...
	\end{itemize}%\todo{state explicitly that we cannot synchronize CTMC and GSMP commands!}
\end{enumerate}
...
%
%Using the value $\lambda_j$ associated with each update $u_j$, the command $c$ defines, for each $s \in S_c$, a function $\mu_{c,s} \colon S \to \Rsetpo$ where for each $t \in S$:
%
